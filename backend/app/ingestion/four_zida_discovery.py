...
from app.sources.four_zida.adapter import (
    DEFAULT_SEARCH_URLS,
    FourZidaAdapter,
    FourZidaConfig,
    SourceFetchError,
...
async def run_four_zida_crawl_async(
    ...
    *,
    ...
    adapter: FourZidaAdapter | None = None,
    ...
    mode = CrawlMode(mode)
    source = ensure_four_zida_source(session)
    ...
    if mode == CrawlMode.DEEP_RECONCILIATION:
        return await _run_four_zida_deep_reconciliation_async(
            session,
            source=source,
    ...
    job_run = JobRun(
        job_type=job_type or _job_type(mode),
        source=source,
    ...
    active_adapter = adapter or FourZidaAdapter()
    ...
    try:
        ...
        cards_seen = sum(len(page.parsed_page.cards) for page in discovery.pages)
        ...
        for index, card in enumerate(discovery.cards):
            ...
            result = _persist_listing(session, source, card, detail, _utcnow())
            if result.is_new:
                new_listings += 1
...
async def run_four_zida_discovery_async(
    ...
    *,
    ...
    return await run_four_zida_crawl_async(
        session,
        mode=CrawlMode.FAST_DISCOVERY,
        job_type="four_zida_manual_discovery",
        adapter=adapter,
...
def run_scheduled_four_zida_crawl(
    ...
    *,
    ...
    return asyncio.run(
        run_scheduled_four_zida_crawl_async(
            session,
            mode=mode,
...
async def _run_four_zida_deep_reconciliation_async(
    session: Session,
    *,
    source: Source,
    runtime_state: SourceRuntimeState,
    adapter: FourZidaAdapter | None,
    ...
    runtime_state.last_attempt_at = started_at
    job_run = JobRun(
        job_type=_job_type(CrawlMode.DEEP_RECONCILIATION),
        source=source,
        started_at=started_at,
        status="RUNNING",
    )
    ...
    candidates = [
        ...
        if _listing_in_current_target_scope(listing)
    ]
    ...
    *,
    ...
    for listing in candidates:
        if listing.external_listing_id in observed_listing_ids:
            continue
        if not _listing_in_current_target_scope(listing):
            continue
        missing_count += 1
        old_status = listing.status
...
def _listing_in_current_target_scope(listing: Listing) -> bool:
    if listing.size_m2 is None:
        return False
    if listing.size_m2 < TARGET_SIZE_MIN or listing.size_m2 > TARGET_SIZE_MAX:
        return False
    location_text = " ".join(
        value
        for value in (
    ...
    return (
        "zemun" in location_text
        or "novi-beograd" in location_text
        or "novi beograd" in location_text
    )
...
def _card_from_detail(detail: RawListingDetail) -> RawListingCard:
    return RawListingCard(
        ...
        additional_card_data={"source": "deep_reconciliation_detail"},
    )
...
def _job_type(mode: CrawlMode) -> str:
    return f"four_zida_{mode.value.lower()}"
...
async def run_four_zida_crawl_loop_async(
    ...
    *,
    ...
    for iteration in range(iterations):
        summary = await run_four_zida_crawl_async(
            session,
            mode=mode,
            adapter=adapter,
            max_pages_per_market=max_pages_per_market,
...
def run_four_zida_crawl_loop(
    ...
    *,
    ...
    return asyncio.run(
        run_four_zida_crawl_loop_async(
            session,
            mode=mode,
...
def _main() -> None:
    ...
    with SessionLocal() as session:
        ...
        if args.iterations == 1:
            ...
            summaries = run_four_zida_crawl_loop(
                session,
                mode=mode,
                adapter=adapter,
